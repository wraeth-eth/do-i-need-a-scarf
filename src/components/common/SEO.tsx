import config from '@config/seo_meta'

import Head from 'next/head'
import { FC, ReactNode } from 'react'
import { OpenGraphMetaTags } from './OpenGraphMetaTags'
import { TwitterMetaTagsProps, TwitterMetaTags, TwitterCardType } from './TwitterMetaTags'
...
export const SEO: FC<Props> = ({ url, title, description, twitter, robots, children }) => {
  const formatTwitterHandle = (handle: string | undefined) =>
    handle ? (handle.startsWith('@') ? handle : '@' + handle) : undefined
  const formattedTitle = title ? `${config.titleTemplate.replace(/%s/g, title)}` : config.title
  return (
    <>
      <Head>
      ...
      <TwitterMetaTags
        ...
        creator={formatTwitterHandle(twitter?.creator ?? config.twitter.creator)}
